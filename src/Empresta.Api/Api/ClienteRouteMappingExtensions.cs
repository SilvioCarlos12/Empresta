...
namespace Empresta.Api.Api
{
    public static class ClienteRouteMappingExtensions
    {
        ...
        {
            app.MapGet($"{rotaRaiz}/{{id}}/{Cliente}",
                    async ([FromServices] IMediator mediator, Guid id, CancellationToken cancellationToken) =>
                    {
                        var response = await mediator.Send(new BuscarClientesPorFuncionarioIdQuery(id),
                            cancellationToken);
                        return response switch
                        {
                ...
                .WithMetadata(new SwaggerOperationAttribute("Obtem clientes de um funcionario",
                    "Obter clientes de um funcionario"))
                .WithTags(WithTags);
        ...
        {
            app.MapPut($"{rotaRaiz}/{{funcioarioId}}/{Cliente}/{{clienteId}}", async ([FromServices] IMediator mediator,
                    Guid funcioarioId, Guid clienteId,
                ...
                .WithMetadata(new SwaggerOperationAttribute("Atualizar um cliente de um funcionario",
                    "Atualizar um cliente de um funcionario"))
                .WithTags(WithTags);
